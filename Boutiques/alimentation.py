...
conn = sqlite3.connect("C:/Users/winny/Desktop/Projet Python POO/BasesDeDonnes/alimentation.db")
c = conn.cursor()
...
class Alimentation:
    ...
    def __init__(self,root) -> None:
        self.root = root
        self.root.title("Boutique d'Alimentation")
        ...
        self.title = Label(self.root, text="ALIMENTATION GÉNÉRALE", font=("Algerian",32), bg="#00d1ff", fg="black")
    ...
    def chargerPrix(self, even=""):
        ...
        import os 
        if os.path.isfile(f"C:/Users/winny/Desktop/Projet Python POO/images/alimentation/{prod.image}") :
            self.produit_image = ImageTk.PhotoImage(Image.open(f"C:/Users/winny/Desktop/Projet Python POO/images/alimentation/{prod.image}"))
    ...
    def retourner(self):
        ...
        self.root.destroy()
        self.root.quit()
        Alimentation.tabStock.clear()
        Alimentation.tabFacture.clear()
        Alimentation.paiement = False
        Alimentation.adresse = ""
        Alimentation.numero = ""
    ...
    def bienvenue(self):
        ...
        self.textarea.insert(END, "\n\t   ** Bienvenue à ALIMENTATION GÉNÉRALE **")
        self.textarea.insert(END, "\n\t\t\t------------------------")
    ...
    def ajouterPanier(self):
        if self.produit.get() =="" :
            ...
            Alimentation.brut = self.totalbrut.get()
            Alimentation.taxe = self.taxe.get()
            Alimentation.net = self.totalnet.get()

            #Enregistrer les achats pour la facture
            fact = Facture(self.produit.get(),self.qte.get(),montant,montant*0.18)
            Alimentation.tabFacture.append(fact)
            ...
            Alimentation.tabStock.append(objStock)
    ...
    def  genererFacture(self):
        if self.clickSurGenerer == False :
            if self.totalbrut.get() == 0 :
                ...
                s = 0
                if Alimentation.livraison == True:
                    self.textarea.insert(END, "\n***************************************************************")
    ...
    def ValiderPanier(self):
        if self.totalbrut.get() == 0 :
            ...
            self.root.destroy()
            self.root.quit()
            if(Alimentation.livraison == False):
                pass
            else:
            ...
            Alimentation.paiement = True # Il entame le paiement
            from Boutiques.mall import Mall
    ...
    def Facture(clss):
        ...
        with open(f"C:/Users/winny/Desktop/Projet Python POO/Factures/{nomFichier}","w",encoding='UTF-8') as f:
            ...
            f.write("\n--------------------------------------------------------------------------------")
            ### Boucle produit
            for i in Alimentation.tabFacture :
                f.write(f"""\n{i.produit:<20}{i.qte:<20}{i.montant:<20}{i.tva:<20}""")
                f.write("\n--------------------------------------------------------------------------------")
            if Alimentation.livraison == True:
                f.write("\n********************************************************************************")
                f.write(f"""\n{"Livraison : ":<20}{2500.0:>40}""")
                f.write(f"""\n{"Adresse : ":<20}{Alimentation.adresse:>40}""")
                f.write(f"""\n{"Numéro du destinataire : ":<30}{Alimentation.numero:>30}""")
            f.write("\n********************************************************************************")
            f.write(f"""\n{"TOTAL BRUT : ":<60}{Alimentation.brut+2500}""")
            f.write(f"""\n{"TAXE : ":<60}{Alimentation.taxe}""")
            f.write(f"""\n{"TOTAL NET : ":<60}{Alimentation.net+2500}""")
            f.write("\n*****************************  MERCI ET À BIENTÔT ******************************")

        #### Insertion dans la base de données Ventes 
        vente = Vente(len(tabVentes),Mall.user.id,f"FACT{num}",Alimentation.id)
        tabVentes.append(vente)
        conn = sqlite3.connect(f"C:/Users/winny/Desktop/Projet Python POO/BasesDeDonnes/ventes.db")
        c = conn.cursor()
       
        param = {"i": Mall.user.id, "l":f"FACT{num}", "m":Alimentation.id}
        c.execute("""
    ...
    def rechercheVente(self):
        from Boutiques.mall import Mall
        for v in tabVentes :
            if ( Mall.user.id==v.idClient and self.factSaisie.get().upper()==v.numFacture and v.idBoutique == Alimentation.id ) :
                return True
