...
class Mode:
    ...
    def __init__(self,root) -> None:
        self.root = root
        self.root.title("Magasin de Mode")
        ...
        self.title = Label(self.root, text="MAGASIN DE MODE", font=("Algerian",32), bg="#00d1ff", fg="black")
        ...
        self.vue_image = ImageTk.PhotoImage(Image.open("C:/Users/winny/Desktop/Projet Python POO/images/modes.jpg"))
    ...
    def chargerPrix(self, even=""):
        ...
        import os 
        if os.path.isfile(f"C:/Users/winny/Desktop/Projet Python POO/images/mode/{prod.image}") :
            self.produit_image = ImageTk.PhotoImage(Image.open(f"C:/Users/winny/Desktop/Projet Python POO/images/mode/{prod.image}"))
    ...
    def ajouterPanier(self):
        if self.produit.get() =="" :
            ...
            Mode.brut = self.totalbrut.get()
            Mode.taxe = self.taxe.get()
            Mode.net = self.totalnet.get()

            #Enregistrer les achats pour la facture
            fact = Facture(self.produit.get(),self.qte.get(),montant,montant*0.18)
            Mode.tabFacture.append(fact)
    ...
    def ValiderPanier(self):
        if self.totalbrut.get() == 0 :
            messagebox.showerror("Error", f"Veuillez choisir au moins un produit !!!") 
        else :
            self.root.destroy()
            self.root.quit()
            if(Mode.livraison == False):
                pass
            else:
            ...
            Mode.paiement = True # Il entame le paiement
            from Boutiques.mall import Mall
    ...
    def Facture(clss):
        ...
        with open(f"C:/Users/winny/Desktop/Projet Python POO/Factures/{nomFichier}","w",encoding='UTF-8') as f:
            ...
            f.write("\n--------------------------------------------------------------------------------")
            ### Boucle produit
            for i in Mode.tabFacture :
                f.write(f"""\n{i.produit:<20}{i.qte:<20}{i.montant:<20}{i.tva:<20}""")
                f.write("\n--------------------------------------------------------------------------------")
            if Mode.livraison == True:
                f.write("\n********************************************************************************")
                f.write(f"""\n{"Livraison : ":<20}{2500.0:>40}""")
                f.write(f"""\n{"Adresse : ":<20}{Mode.adresse:>40}""")
                f.write(f"""\n{"Numéro du destinataire : ":<30}{Mode.numero:>30}""")
            f.write("\n********************************************************************************")
            f.write(f"""\n{"TOTAL BRUT : ":<60}{Mode.brut+2500}""")
            f.write(f"""\n{"TAXE : ":<60}{Mode.taxe}""")
            f.write(f"""\n{"TOTAL NET : ":<60}{Mode.net+2500}""")
            f.write("\n*****************************  MERCI ET À BIENTÔT ******************************")

        #### Insertion dans la base de données Ventes 
        vente = Vente(len(tabVentes),Mall.user.id,f"FACT{num}",Mode.id)
        tabVentes.append(vente)
        conn = sqlite3.connect(f"C:/Users/winny/Desktop/Projet Python POO/BasesDeDonnes/ventes.db")
        c = conn.cursor()
       
        param = {"i": Mall.user.id, "l":f"FACT{num}", "m":Mode.id}
        c.execute("""
    ...
    def rechercheVente(self):
        from Boutiques.mall import Mall
        for v in tabVentes :
            if ( Mall.user.id==v.idClient and self.factSaisie.get().upper()==v.numFacture and v.idBoutique == Mode.id ) :
                return True
